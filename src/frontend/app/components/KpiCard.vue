<template>
  <div class="glass-card rounded-xl p-5 border border-[var(--border-color)] relative overflow-hidden group">
    ...
    <div class="flex items-center justify-between mb-3">
      <span class="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">{{ title }}</span>
      <div
    ...
    <div class="flex items-baseline gap-2">
      <span class="text-2xl font-bold tracking-tight font-mono-num text-[var(--text-main)]">{{ value }}</span>
      <span v-if="change" class="text-xs font-medium px-1.5 py-0.5 rounded" :class="changeIsPositive ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-500 border border-rose-500/20'">
