  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-card p-5 rounded-xl border border-[var(--border-color)]">
      <div>
        ...
        <p class="text-xs text-[var(--text-muted)] mt-0.5">Hierarchical general ledger accounts linked to fund accounts</p>
      </div>
    ...
    <div class="glass-card rounded-xl border border-[var(--border-color)] overflow-hidden">
      <div class="p-4 border-b border-[var(--border-color)] flex items-center justify-between gap-4">
        <div class="text-xs font-semibold text-[var(--text-main)] uppercase tracking-wider">Active General Ledger Accounts</div>
        <input
          v-model="searchQuery"
          type="text"
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-[var(--border-color)] text-xs">
              <tr v-for="acc in filteredAccounts" :key="acc.id" class="hover:bg-[var(--bg-table-hover)] transition-colors">
                <td class="p-3.5 font-mono font-bold text-emerald-500">{{ acc.account_code }}</td>
                <td class="p-3.5 font-semibold text-[var(--text-main)]">{{ acc.account_name }}</td>
                <td class="p-3.5">
                  <span class="bg-blue-500/10 text-blue-500 border border-blue-500/20 px-2 py-0.5 rounded font-mono text-[11px]">
                    {{ getFundCode(acc.fund_account_id) }}
                  </span>
                </td>
                <td class="p-3.5 text-[var(--text-muted)] max-w-md truncate">{{ acc.description || 'N/A' }}</td>
                <td class="p-3.5 text-right font-mono text-[var(--text-muted)] text-[11px]">{{ acc.created_at }}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <Modal :isOpen="isModalOpen" title="Create New Ledger Account" @close="isModalOpen = false">
      <form @submit.prevent="handleCreateAccount" class="space-y-4">
        ...
        <div>
          <label class="block text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1">Parent Fund Account *</label>
