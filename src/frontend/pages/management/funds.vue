  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-card p-5 rounded-xl border border-[#1E293B]">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-0.5">Corporate fund accounts linked to {{ auth.currentCompany.value?.business_name }}</p>
      </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div
        ...
        :key="fund.id"
        class="glass-card p-5 rounded-xl border border-[#1E293B] relative overflow-hidden flex flex-col justify-between group"
      >
        <div>
          <div class="flex items-center justify-between mb-3">
            <span class="px-2.5 py-1 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-mono text-xs font-bold">
              {{ fund.fund_code }}
            </span>
            <span class="text-[11px] text-slate-500 font-mono">ID: #{{ fund.id }}</span>
          </div>

          <h3 class="text-base font-bold text-slate-100 group-hover:text-emerald-400 transition-colors">
            {{ fund.fund_name }}
          </h3>
        ...
        <div class="pt-4 mt-4 border-t border-[#1E293B] flex items-center justify-between text-xs">
          <div class="text-slate-500">
            Created: <span class="font-mono text-slate-300">{{ fund.created_at }}</span>
          </div>
          <span class="text-emerald-400 font-medium">Active Fund</span>
        </div>
      </div>
    </div>
    ...
    <Modal :isOpen="isModalOpen" title="Create New Fund Account" @close="isModalOpen = false">
      <form @submit.prevent="handleCreateFund" class="space-y-4">
        <div>
          <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1">Fund Code *</label>
          <input v-model="newFund.fund_code" type="text" required placeholder="FND-404" class="input-field font-mono" />
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-1">Fund Name *</label>
...
const handleCreateFund = async () => {
  await accounting.addFundAccount({
    ...newFund,
